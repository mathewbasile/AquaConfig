...
// example use of header only crc
// CRC<Crc16> polar_cs;

enum CrcTypes { CheckSum, Crc16, Crc32 };
...
static const uint16_t crc16_3D65[] =
{
...
static const uint16_t crc32_precompiled_0xEDB88320_reverse[] =
{
...
*/
struct interfaceCRC {
	virtual uint32_t GetCrc(const unsigned char buf[], uint32_t len) = 0;
};

/**
	 @brief  template that takes enum of crc wanted
	 only one function needed
*/
template<CrcTypes N_type >
struct CRC  {
	unsigned int GetCrc(const unsigned char buf[], uint32_t len);
};

/**
	 @brief  create unique class for checksum, that overrides base 
	 only one function needed
*/
struct checksum : interfaceCRC
{
	uint32_t GetCrc(const unsigned char buf[], uint32_t len)
	{
		unsigned int crc = -1;
...
/**
	 @brief  specialize template for checksum version
*/
template<>
struct CRC<CheckSum> : public checksum {
	uint32_t GetCrc(const unsigned char buf[], uint32_t len)
	{
		return checksum::GetCrc(buf, len);
	}
};

struct crc16 : interfaceCRC
{
	uint32_t GetCrc(uint8_t const message[], uint32_t nBytes)
	{
		uint8_t data;
		crc remainder = 0;
		...
		for (int byte = 0; byte < nBytes; ++byte)
		{
			...
			remainder = crc16_3D65[data] ^ (remainder << 8);
		}
...
template<>
struct CRC<Crc16> : public crc16 {
	unsigned int GetCrc(const uint8_t buf[], uint32_t len)
	{
		return crc16::GetCrc(buf, len);
	}
};

struct crc32 : interfaceCRC
{
	
};

template<>
struct CRC<Crc32> : public crc32 {
	unsigned int GetCrc(const unsigned char buf[], uint32_t len)
	{
		return crc32::GetCrc(buf, len);
	}
};
